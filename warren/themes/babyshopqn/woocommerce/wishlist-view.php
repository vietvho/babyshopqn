...
<?php if ($atts['layout'] =='grid'): ;?>
	...
	$items = array_slice($wishlist_items,0,4);
		?>
	<section class="c-wishlist-grid">
		<ul class="products">
		<?php foreach ( $items as $item ) : 
			...
			?>
			<li class="product c-wishlist-grid__item">
				<a class="c-wishlist-grid__image woocommerce-LoopProduct-link" href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>">
					<?php echo $product->get_image(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<?= '<h2>'.$product->get_title().'</h2>';?>
				</a>
				<div class="product-desc">
					<?php
					if ($product->is_type( 'simple' ) ){ echo $product->get_short_description();}
			...
			<?php endif; //product exist?>
		<?php endforeach;?>
		</ul>
	</section>
	<?php else: ?>
	<p><strong><?php _e('Bạn chưa có sản phẩm nào trong danh sách yêu thích','bbs');?></strong></p>
	
	<?php if (!is_user_logged_in()){?>
		<br>
		<p><?php _e('Đăng kí thành viên để đặt hàng nhanh và nhận được nhiều ưu đãi hơn!','bbs');?></p>
		<br><br>
		<?php 
		echo '<p><a class="btn" href="'.get_permalink( get_option('woocommerce_myaccount_page_id') ).'">'.__('Log in','bbs').'</a></p>';
		}
		?>
	<?php endif; // wishlist has items?>
			...
			<?php foreach ( $wishlist_items as $item ) : 
				...
				?>
					<div class="wishlist-list__item">
						<a class="wishlist-list__item-image" href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>">
							<?php echo $product->get_image([150,150]); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
						</a>
						<div class="wishlist-list__item-content">
							<?= '<h2><a href="'. esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ).'">'.$product->get_title().'</a></h2>';?>
							<?php
							if ($product->is_type( 'simple' ) ){ echo '<p>'.$product->get_short_description().'</p>';}
							if ( $show_variation && $product->is_type( 'variation' ) ) {
								// phpcs:ignore Generic.Commenting.DocComment
								/**
									* @var $product \WC_Product_Variation
							...
							?>
							<div class="wishlist-list__price">
							<?php
							if ( $show_price ) {
							...
							<?php if ( $show_add_to_cart && isset( $stock_status ) && 'out-of-stock' !== $stock_status ) : ?>
								<?php woocommerce_template_loop_add_to_cart( array('class'=>'btn btn--outlined', 'quantity' => $show_quantity ? $item->get_quantity() : 1 ) ); ?>
							<?php endif ?>
						</div>
						<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove_from_wishlist" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'yith-woocommerce-wishlist' ) ) ); ?>"><i class="yith-wcwl-icon fa fa-times"></i></a>
					</div>
				<?php endif; // if have product;?>
			<?php endforeach; // foreach wishlist item?>
		</div> <!-- end wishlist -->
		<?php else: ?>
			<div class="wishlist--no-items">
				<?php if (!is_user_logged_in()){?>
				<p><?php _e('Đăng kí thành viên để đặt hàng nhanh và nhận được nhiều ưu đãi hơn!','bbs');?></p>
				<div class="c-wishlist__button-group u-mt-20">
					<?php 
					echo '<a class="btn btn__login c-btn--show-modal u-mr-5 btn--inline" href="javascript:void(0)" data-target="#formLogin">'.__('Đăng nhập','bbs').'</a>';
					echo '<a class="btn btn__signup c-btn--close-modal btn--outlined c-btn--show-modal btn--inline mt-10"  data-target="#formRegister" href="javascript:void(0)">'.__('Đăng ký','bbs').'</a>'; ?>
				</div>
				<?php } else 
				{
					echo '<p>'.__('Bạn chưa có sản phẩm nào trong danh sách yêu thích','bbs').'</p>';
				} ?>
			</div>
		<?php endif; // wishlist has items?>
<?php else:?> <!-- layout -->

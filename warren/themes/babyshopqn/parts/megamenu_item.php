			...
			while ( have_rows( 'mega_menu', $_post_id ) ) {
			    ...
			    if($row['is_enable']):
			    	...
			    	if($row['menuitem_select'] === 'link') {
				    	...
				    	$_attr_callmegamn = ($row['is_show_submenu']) ? 'data-id_nam="' . $_name . '"' : '';
				    	$_target = isset($_mn_item['target']) && !empty($_mn_item['target']) ? $_mn_item['target'] : '_self';
				    	$_target = $_mn_item['target'] ?: '_self';
					    ?>
							<li class="menu-item" <?= $_attr_callmegamn ?> ><a href="<?= $_mn_item['url'] ?>" target="<?= $_target ?>" ><?= $_mn_item['title'] ?></a></li>
					    <?php 
			    	} else {
				    	...
				    	$_attr_callmegamn = ($row['is_show_submenu']) ? 'data-id_nam="' . $_name . '"' : '';
					    ?>
							<li class="menu-item" <?= $_attr_callmegamn ?> ><span ><?= $_title ?></span></li>
					    <?php 
			    	}
			    endif;
			}

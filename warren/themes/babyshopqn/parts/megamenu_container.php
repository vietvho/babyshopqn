<?php 
if($row['is_enable'] && $row['is-show_submenu'] && $row['add_column'] && count($row['add_column'])>0):
...
<div class="c-megamenu" data-class="<?= $_name ?>">
	<div class="c-megamenu__content">
		<div class="bbs2-container">
			<p class="c-megamenu__viewalllink"><?= __('Xem tất cả', 'bbs') ?>: 
			<?php if(!empty($_url)): ?>
			...
			<div class="c-megamenu__row" data-ncolumn="<?= count($row['add_column']) ?>">
				<?php 
				...
				<?php 
					elseif($_v['type_item'] === 'relatedbanner'):
						...
						if($_image !== false && $_link !== false ) {
							?>
							<div class="c-megamenu__col mn_related_img">
